...
import { getDefaultBreadcrumbUrl, BuildHelpLink } from "Build/Scripts/Linking";
...
import { getPermissionsStore, IPermissionsStore } from "Build/Scripts/Stores/Permissions";
import { Features, Sources, publishEvent } from "Build/Scripts/Telemetry";

import { BuildPermissions, BuildSecurity } from "Build.Common/Scripts/Generated/TFS.Build2.Common";
...
export class TitleBar extends React.Component<Props, State> {
    ...
    public render(): JSX.Element {
        const breadcrumbs = getBreadcrumbs(this.state.path);

        let headingElement: JSX.Element = null;
        if (breadcrumbs.length === 0) {
            headingElement = <div className="title-heading expand">
                <h1 className="ms-font-l">{BuildResources.BuildDefinitionsTitle}</h1>
            </div>;
        }
        else {
            headingElement = <div className="title-heading">
                <BreadcrumbLink
                    path={"\\"}
                    title={BuildResources.BuildDefinitionsTitle}
                    onBreadcrumbClicked={this._onBreadcrumbClicked}
                    getBreadcrumbLink={getDefaultBreadcrumbUrl} />
            </div>;
        }
