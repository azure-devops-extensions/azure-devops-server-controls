...
SDK_Shim.registerContent("buildagentPoolView.initialize", (context: SDK_Shim.InternalContentContextData): IDisposable => {

    const pageDataService = Service.getService(Contribution_Services.WebPageDataService);
    const pageData = pageDataService.getPageData<AdminAgentPoolComponentProps>("ms.vss-build-web.agent-pool-hub-data-provider");
    ...
    Promise.all(loadPromises).then(() => {        
        ...
        ReactDOM.render(
            <AdminAgentPoolComponent {...pageData} />,
            context.container);
    });
...
interface AdminAgentPoolComponentProps {    
    jQuerySignalrUrl: string;
    signalrHubUrl: string;
    timeZoneData: string;
}

class AdminAgentPoolComponent extends React.Component<AdminAgentPoolComponentProps, {}> {

    private createAgentPoolDialogTemplate = `<div class="queue-build">
    <table class="filter">
        <tr>
            <td colspan="2">
        ...
        <tr data-bind="if: showPipelineAuthorization">
            <td colspan="2">
