...
/**
 * Information to use in constructing the url.  The object has the following structure:
 * {
 *     team: Defaulted to the current team in the navigation system if not provided.
 *     includeTeam: Defaulted to true. If it is true, 'team' parameter defined above overrides 'includeTeam' parameter.
 *                            If it is false, 'team' parameter is ignored
 *     project: Defaulted to the current project in the navigation system if not provided.
 *     serviceHost: Defaulted to the current serviceHost (collection) in the navigation system if not provided.
 *     area: Defaulted to the current area in the navigation system if not provided. Ex: "admin", or "api".
 *     includeVersion: When set to true, the web client version will be included as part of the query string for the generated URL.
 ...
 *     useApiUrl: When set to true and project and team information should be added to the uri, 
...
export interface IRouteData {
    team?: string;
    includeTeam?: boolean;
    ignoreDefaultTeam?: boolean;
    project?: string;
    serviceHost?: IServiceHost | any;
    area?: string;
...
export class TfsContext {
    ...
    private static _previousTfsContext: TfsContext;
    private static _addedFpsCallback: boolean;
    ...
    public static getDefault(): TfsContext {
        if (!TfsContext._defaultTfsContext) {
            ...
            TfsContext.setDefaultContext(defaultContextData);
        }
    ...
    public static tryGetDefaultContext(): TfsContext {
        if (typeof TfsContext._defaultTfsContext === "undefined") {
            ...
            if (defaultContextData) {
                ...
                TfsContext._defaultTfsContext = null;
            }
        }
        return TfsContext._defaultTfsContext;
    }

    private static setDefaultContext(defaultContextData: Contracts_Platform.WebContext) {
        // on new platform FPS, _defaultTfsContext gets set to null, but we want to recycle the
        // _previousTfsContext object in case someone has kept a handle to it
        if (TfsContext._previousTfsContext) {
            TfsContext._previousTfsContext.initialize(defaultContextData);
            TfsContext._defaultTfsContext = TfsContext._previousTfsContext;
        }
        else {
            TfsContext._defaultTfsContext = new TfsContext(defaultContextData);
            TfsContext._previousTfsContext = TfsContext._defaultTfsContext;
        }
    }
    ...
    constructor(contextData: Contracts_Platform.WebContext) {
        ...
        if (!TfsContext._addedFpsCallback) {
            // Setup a handler to clear the cached default tfs context after a fast page switch
            TfsContext._addedFpsCallback = true;
            document.body.addEventListener("legacyFpsComplete", () => {
                TfsContext._defaultTfsContext = undefined;
            });
        }

        this.initialize(contextData);
    }

    /**
     * Used to initialize the TfsContext when it's created or after an FPS.
     * @param contextData
     */
    public initialize(contextData: Contracts_Platform.WebContext): void {
        ...
        this.navigation = {
            ...
            currentParameters: defaultPageContext.navigation.currentParameters,
            project: null,
            projectId: null,
            publicAccessPoint: {
     ...
     *     includeTeam: OPTIONAL: Defaulted to tue. If it is true, 'team' parameter defined above overrides 'includeTeam' parameter.
     *                            If it is false, 'team' parameter is ignored
     ...
     * @return 
     */
    public getPublicActionUrl(action: string, controller: string, routeData?: IRouteData): string {
     ...
     * @return 
     */
    public getActionUrl(action?: string, controller?: string, routeData?: IRouteData): string {
        if (!this.isSameHost) {
     ...
     * @return 
     */
    public getCollectionActionUrl(collectionName: string, action: string, controller?: string, routeData?: IRouteData): string {
        var collectionContext = this._getCollectionTfsContext(collectionName);
        return collectionContext.getActionUrl(action, controller, routeData);
    }
    ...
    /**
     * Returns a duplicate tfscontext overriding the service host for a given collection name
     * 
     * @param collectionName colllectionName
     * @return 
     */
    private _getCollectionTfsContext(collectionName: string): TfsContext {
        Diag.Debug.assertParamIsNotNull(collectionName, "collectionName is null");
        if (this.navigation.serviceHost.hostType === TeamFoundationHostType.ProjectCollection &&
            this.navigation.serviceHost.name.toLowerCase() === collectionName.toLowerCase()) {
            return this;
        }
        else {
            ...
            return this.getCollectionTfsContext(serviceHost);
        }
    }
     ...
     * @return 
     */
    public getPermalinkUrl(action: string, controller?: string, routeData?: IRouteData): string {
    ...
    public getIdentityImageUrl(identityId: string, urlParams?: any): string {
        const identityDetails = LocalPageData.getSharedData("_identityDetails");
        const defaultParams = {
            area: TfsContext._API,
            project: "",
            id: identityId,
            t: (identityDetails && identityDetails[identityId]) ? identityDetails[identityId].identityImageId : imageTimeStamp,
        };
     ...
     * @return 
     */
    private _constructActionUrl(action?: string, controller?: string, routeData?: IRouteData): string {
        ...
        if (serviceHost) {
            ...
            if (serviceHost.hostType === TeamFoundationHostType.ProjectCollection) {
                ...
                if (project) {
                    ...
                    if (typeof routeParams.includeTeam === "undefined" || routeParams.includeTeam === null) {
                        routeParams.includeTeam = true;
                    }

                    if (routeParams.includeTeam) {
        ...
        delete routeParams.includeTeam;
     ...
     * @return 
     */
    private _constructPermalinkUrl(action: string, controller?: string, routeData?: IRouteData): string {
