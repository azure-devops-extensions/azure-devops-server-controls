...
import "VSS/LoaderPlugins/Css!TestManagement/Scripts/Scenarios/BugsGridView/ControllerViews/BugsGridView";
...
import { BugsGridViewStore, IBugsGridViewState } from "TestManagement/Scripts/Scenarios/BugsGridView/Stores/BugsGridViewStore";
...
export interface IBugsGridViewProps extends ComponentBase.Props {
    actionsCreator: BugsGridViewActionsCreator;
    store: BugsGridViewStore;
    testCaseResult: TCMContracts.TestCaseResult;
}

export function renderGrid(element: HTMLElement, BugsGridViewProps: IBugsGridViewProps): void {
    ReactDOM.render(<BugsGridView { ...BugsGridViewProps } />, element);
}
...
export class BugsGridView extends ComponentBase.Component<IBugsGridViewProps, IBugsGridViewState> {
    ...
    public componentDidMount(): void {
        this.props.actionsCreator.loadAssociatedBugs(this.props.testCaseResult);
    ...
    public render(): JSX.Element {
        let { columns, bugs } = this.state;
        return (
            <Fabric className="test-bugs-grid-view">
                <h2 className="bugs-header">
                    {Utils_String.format(Resources.BugsLabel, (bugs) ? bugs.length : 0)}
                </h2>
                {
                ...
                }
                {
                    this.state.bugs && this.state.bugs.length > 0 ?
                        <DetailsList
                            className="bugs-grid"
                            items={this.state.bugs}
                            ariaLabelForGrid={Resources.BugsText}
    ...
    private _showLoading(): JSX.Element {
        return (
            <div className="bugs-view-pane-loading">
                <Spinner
                    ariaLabel={Resources.LoadingBugsMessage}
                    className="bugs-grid-view-loading-spinner test-result-loading-spinner-separator"
                    size={SpinnerSize.medium}
                    label={Resources.LoadingBugsMessage}
                />
            </div>
        );
    }
    ...
    private _onColumnHeaderClick(ev: any, column: IColumn) {
        const { columns, bugs } = this.state;
        let newItems: WIT_Contracts.WorkItem[] = bugs.slice();
        ...
        this.props.actionsCreator.afterSortBugs(newItems);
    ...
    private _sortItems(items: WIT_Contracts.WorkItem[], sortBy: string, descending = false): WIT_Contracts.WorkItem[] {
        TelemetryService.publishEvents(TelemetryService.featureBugsGridView_sortOnBugs, {
    ...
    private _createContextMenu(bug: WIT_Contracts.WorkItem, index: number): JSX.Element {
        ...
        let contextMenuProps: IPopupContextualMenuProps = {
            ...
            onClick: (event) => { this._onContextMenuClick(bug, index, event); },
    ...
    private _onItemInvoked = (item: WIT_Contracts.WorkItem, event: any): void => {
        if (event.type === "keypress" && event.key === "Enter" || event.type === "click") {
            if (event.ctrlKey) {
                this.props.actionsCreator.openBug(item);
                return;
            }
            let options = {
                save: (workItem) => {
            ...
            TelemetryService.publishEvents(TelemetryService.featureBugsGridView_TestResultViewBug, {
    ...
    private _openBug(item: WIT_Contracts.WorkItem): void {
        this.props.actionsCreator.openBug(item);
        TelemetryService.publishEvents(TelemetryService.featureBugsGridView_TestResultViewBug, {
            "ViewAction": "Open on new tab",
    ...
    private _onContextMenuClick(item?: any, index?: number, ev?: any): void {
        ...
        if (this.state.selectionDetails != null) {
            this.state.selectionDetails.forEach((bug) => {
                if (item.id === bug.id) {
    ...
    private _onItemContextMenu(item?: any, index?: number, ev?: Event): void {
        $(".bugs-grid-context-menu").eq(index).find(".popup-menu-trigger").click();
    }
