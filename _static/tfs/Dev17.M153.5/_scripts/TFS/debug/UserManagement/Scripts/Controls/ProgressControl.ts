...
export class ProgressControl extends Controls.BaseControl {
    ...
    private _draw(current: number, total: number, maxTotal: number, text: string, options?: any) {
        ...
        if (options.summary) {
            ...
            if (total === 0) {
                stringWithSuffix = Utils_String.format(AccountResources.ProgressControl_NoCurrentValue, options.suffixFormat);
                progressText = Utils_String.format(stringWithSuffix, Helpers.FormatUtils.formatNumberForDisplay(current, ProgressControl.DECIMAL_PRECISION));
            }
            else {
                stringWithSuffix = Utils_String.format(options.suffixFormat, Helpers.FormatUtils.formatNumberForDisplay(total, ProgressControl.DECIMAL_PRECISION));
                progressText = Utils_String.format(AccountResources.ProgressControl_DefaultProgressText, Helpers.FormatUtils.formatNumberForDisplay(current, ProgressControl.DECIMAL_PRECISION), stringWithSuffix);
            }

            $progressTextDiv.text(progressText)
                .addClass("progress-text")
                .attr("title", progressText);
    ...
    private _drawSummaryStyle(current: number, total: number, maxTotal: number, text: string, options?: any) {
        ...
        //Set the capacity info text
        totalWithSuffix = Utils_String.format(options.suffixFormat, Helpers.FormatUtils.formatNumberForDisplay(total, ProgressControl.DECIMAL_PRECISION));
        currentText = Helpers.FormatUtils.formatNumberForDisplay(current, ProgressControl.DECIMAL_PRECISION);
        progressText = Utils_String.format(AccountResources.ProgressControl_SummaryProgressText, totalWithSuffix);
        toolTip = Utils_String.format(AccountResources.ProgressControl_SummaryProgressToolTip, currentText, totalWithSuffix);
