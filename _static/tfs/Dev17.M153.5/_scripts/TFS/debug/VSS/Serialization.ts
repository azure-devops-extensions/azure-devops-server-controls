...
export interface ContractEnumMetadata {
    enumValues?: { [enumName: string]: number };
}
...
export module ContractSerializer {
    ...
    function _getTranslatedEnumValue(enumType: ContractEnumMetadata, valueToConvert: any, serialize: boolean): any {
        ...
        else if (!serialize && typeof valueToConvert === "string" && isNaN(parseInt(valueToConvert))) {
            // Deserialize: String --> number.
            // Note that integer dictionary keys (keys already deserialized) will still be in string form, so we check above for NaN
            var result = 0;
            if (valueToConvert) {
...
export function deserializeVssJsonObject<T>(text: string): T {
    ...
    if (text) {
        ...
        let replacedText = text.replace(msAjaxDateRegEx, "$1{\"__msjson_date__\":$2 }");
