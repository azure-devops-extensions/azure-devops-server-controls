...
import { NumericValueRange, CategoryRange, ValueSpinner } from "WorkItemTracking/Scripts/Form/React/Components/ValueSpinner";

export interface ITimeProps {
    /**
     * Callback issued when a time is changed
     * @param hour - The hour in 24 hour format (from 0 to 23)
    ...
    /**
     * if true, render AM/PM before hour and minute
     */
    renderAmPmBeforeTime?: boolean;

    /**
     * time strings that should be localized
     */
...
export interface ITimeStrings {
    AMDesignator: string;
    PMDesignator: string;
    ariaLabelHourUpButton: string;
    ariaLabelHourDownButton: string;
    ariaLabelMinuteUpButton: string;
    ariaLabelMinuteDownButton: string;
    ariaLabelAMPMUpButton: string;
    ariaLabelAMPMDownButton: string;
}
...
export class Time extends React.Component<ITimeProps, ITimeState> {
    ...
    private _renderAMPM(): JSX.Element {
        if (!this.props.use24HourFormat) {
            ...
            let value = this.state.isAM ? AMDesignator : PMDesignator;

            return <div className="time-ampm">
                <StringSpinner
                    value={value}
                    valueRange={this._AMPMRange}
                    onValueChange={this._onAMPMChange}
                    ariaLabelUpButton={this.props.timeStrings.ariaLabelAMPMUpButton}
                    ariaLabelDownButton={this.props.timeStrings.ariaLabelAMPMDownButton}
                />
            </div>;
        }
    ...
    private _renderHour(): JSX.Element {
        return <div className="time-hour">
            <NumberSpinner
                ...
                ariaLabelUpButton={this.props.timeStrings.ariaLabelHourUpButton}
                ariaLabelDownButton={this.props.timeStrings.ariaLabelHourDownButton}
            />
        </div>;
    }

    private _renderMinute(): JSX.Element {
        return <div className="time-minute">
            <NumberSpinner
                value={this.state.minute}
                valueRange={this._minuteRange}
                onValueChange={this._onMinuteChange}
                ariaLabelUpButton={this.props.timeStrings.ariaLabelMinuteUpButton}
                ariaLabelDownButton={this.props.timeStrings.ariaLabelMinuteDownButton}
            />
        </div>;
    }
