...
import { QueryDefinition } from "WorkItemTracking/SharedScripts/QueryHierarchy";
...
const WORKITEMPANE_MINIMUM_TOP_OQE = 360;
...
export class QueryEditor extends Controls.BaseControl {
    ...
    private _gridToolbar: QueryResultToolbar;
    ...
    private _resizeDelegate = () => { this.handleResize(); };
    ...
    private _refreshMenuBarDelegate: () => void;
    ...
    constructor(options?) {
        ...
        this._refreshMenuBarDelegate = () => this._refreshMenuBar();
    ...
    public dispose() {
        ...
        Events_Services.getService().detachEvent(WorkItemsProvider.EVENT_QUERY_PROVIDER_DIRTY_CHANGED, this._refreshMenuBarDelegate);

        if (this._gridToolbar) {
            this._gridToolbar.dispose();
        }
    ...
    public setProvider(queryProvider: QueryResultsProvider, callback: IResultCallback, errorCallback?: IErrorCallback, extras?: IQueryParamsExtras) {
        ...
        Events_Services.getService().attachEvent(WorkItemsProvider.EVENT_QUERY_PROVIDER_DIRTY_CHANGED, this._refreshMenuBarDelegate);
    ...
    private handleResize() {
        const splitterMaxSize = this.getSplitterMaxSize();
        this._splitter.setMaxWidth(splitterMaxSize);
        // Update splitter size if it fell outside its boundaries
        if (this._splitter.rightPane.height() > splitterMaxSize) {
            this._splitter.resize(splitterMaxSize, true);
        }
    ...
    public _createElement() {
        ...
        this._toolbarHost = this._createToolbarHost();
        this._menubar = this._createMenuBar(this._toolbarHost);

        const $queryTypeHost = this._createQueryOptionsHost();
        ...
        this._linkFilterModeCombo = <Combos.Combo>Controls.BaseControl.createIn<Combos.IComboOptions>(Combos.Combo, $queryModeHost, {
            ...
            indexChanged: (index: number) => { this._updateQueryMode(); }
        });
        ...
        if (this._options.infoBar === true) {
            this._gridInfoBar = <QueryResultInfoBar>Controls.BaseControl.createIn(
                QueryResultInfoBar, this._splitter.rightPane, { cssClass: "work-item-list-info", tfsContext: this._options.tfsContext });
        } else {
            this._gridInfoBar = <QueryResultInfoBar>this._options.infoBar;
        }

        this._gridToolbar = this._createResultGridToolbar(this._splitter.rightPane);
        ...
        // Register shortcuts for the editor grid for OQE
        new QueryResultGrid.QueryResultGridShortcutGroup(Resources.KeyboardShortcutGroup_Queries, this._grid);
        ...
        if (this._gridToolbar) {
            this._gridToolbar.bind(this._grid);
        }
    ...
    public _createSplitter(): Splitter.Splitter {
        return <Splitter.Splitter>Controls.BaseControl.createIn(Splitter.Splitter, this._element, {
            ...
            maxWidth: this.getSplitterMaxSize(),
            minWidth: this.getSplitterMinSize(),
            fixedSide: "right",
            splitWidth: "350px",
            handleBarWidth: "5px"
        });
    ...
    private getSplitterMaxSize(): number {
        const documentHeight = $(document).height();
        return documentHeight - WORKITEMPANE_MINIMUM_TOP_OQE;
    }

    private getSplitterMinSize(): number {
    ...
    private _createQueryAcrossProjectsCheckbox($host: JQuery) {
        ...
        const $title = $("<label />")
            ...
            .attr("for", id + "_queryAcrossProjectCheckbox")
        ...
        this._queryAcrossProjectsCheckboxControl = $("<input type='checkbox' />")
            ...
            .attr("id", id + "_queryAcrossProjectCheckbox")
            .appendTo($host)
            .bind("change", () => {
    ...
    private _queryAcrossProjectsChanged() {
        ...
        if (queryAcrossProjects) {
            editInfo.teamProject = null;
        } else {
            editInfo.teamProject = this._queryProvider.project.name;
        }
    ...
    public _createToolbarHost() {
        return $("<div />").addClass("toolbar").appendTo(this._splitter.leftPane);
    }
    ...
    public _createResultGridToolbar($containerElement: JQuery): QueryResultToolbar {
        let queryResultToolbar: QueryResultToolbar;
        queryResultToolbar = <QueryResultToolbar>Controls.BaseControl.createIn(
            QueryResultToolbar, $containerElement, { cssClass: "work-item-list-toolbar", tfsContext: this._options.tfsContext, hideFilter: true });
        ...
        if (!this._avoidGettingResults) {
            ...
            const onProjectsLoad = (model: IQueryResult) => {
                ...
                this._grid.beginShowResults(this._queryProvider, () => {
                    ...
                    // Fire queryResultScenarioEnded event to notify QueryResultToolbar to refresh contributed menu items
                    Service.getLocalService(Events_Services.EventService).fire("queryResultScenarioEnded", this);
                ...
                this._queryAdapter.beginEnsureFields(() => {
                    this._queryAdapter.beginGetRecursiveLinkTypes(() => {
                        this._queryAdapter.beginGetOneHopLinkTypes(() => {
                            ...
                            if (scopedProjectId) {
                                this._queryAdapter.store.beginGetProject(scopedProjectId, (project: WITOM.Project) => {
                                    onProjectsLoad(model);
                                }, errorCallback);
                            } else {
                                this._queryAdapter.store.beginGetProjects((projects: WITOM.Project[]) => {
                                    onProjectsLoad(model);
                                }, errorCallback);
                            }
                        }, errorCallback);
    ...
    public _createMenuBar($containerElement: JQuery): Menus.MenuBar {
        ...
        const menuBar = <Menus.MenuBar>Controls.BaseControl.createIn(Menus.MenuBar, $containerElement, {
            items: this._createMenubarItems(),
            executeAction: delegate(this, this._onMenubarItemClick),
            getCommandState: delegate(this, this._getCommandState)
        });
    ...
    public _createMenubarItems() {
        const that = this;
        const items: Menus.IMenuItemSpec[] = [];

        function getActionArgs() {
            return that._onGetMenuItemActionArguments(this);
        }

        items.push({ id: "save-query", text: Resources.SaveQuery, showText: false, title: Resources.SaveQuery, icon: "bowtie-icon bowtie-save" });

        if (this._options.tfsContext.standardAccessMode === true) {
            items.push({
                id: "save-query-as", text: Resources.SaveQueryAs, showText: false, title: Resources.SaveQueryAsTooltip, icon: "bowtie-icon bowtie-save-as", "arguments": () => {
                    return {
                        project: that._queryProvider.queryDefinition.project,
                        querydefinition: that._queryProvider.queryDefinition,
                    };
                }
            });
        }

        items.push({ id: "run-query", text: Resources.RunQuery, showText: false, title: Resources.RunQuery, icon: "bowtie-icon bowtie-status-run" });
        items.push({ id: "revert-query-changes", text: Resources.RevertQueryChanges, showText: false, title: Resources.RevertQueryChanges, icon: "bowtie-icon bowtie-edit-undo", "arguments": getActionArgs });
        items.push({ separator: true });
        items.push({ id: "column-options", text: Resources.ColumnOptions, title: Resources.ColumnOptionsTooltip, noIcon: true, "arguments": getActionArgs, setTitleOnlyOnOverflow: true });
        items.push({ id: "share-link", text: Resources.CopyQueryURL, title: Resources.CopyQueryURLTitle, noIcon: true, setTitleOnlyOnOverflow: true });
        return items;
    }

    private _onMenubarItemClick(e?: CommandEventArgs) {
        ...
        switch (command) {
            ...
            case "run-query":
                this.runQuery();
                return false;
            case "revert-query-changes":
                this.revert();
                return false;
            case "share-link":
                this.getResultsGrid().shareLink();
                return false;
        }
    }

    private _onGetMenuItemActionArguments(menuItem: Menus.MenuItem) {
        const action = menuItem.getAction();
        if (action === "column-options") {
            return this._grid.getActionArguments(action);
        }
    }

    private _getCommandState(command: string) {
        if (command === "save-query") {
            if (!(this._queryProvider && this._queryProvider.isSaveable())) {
                return Menus.MenuItemState.Disabled;
            }
        }
        if (command === "revert-query-changes") {
            if (!this.isDirty()) {
                return Menus.MenuItemState.Disabled;
            }
        }
        if (command === "share-link") {
            if (this._queryProvider &&
                this._queryProvider.queryDefinition &&
                QueryDefinition.isSpecialQueryId(this._queryProvider.queryDefinition.id)) {
                return Menus.MenuItemState.Hidden;
            }
            if (!this._enableCopyQueryCommand) {
                return Menus.MenuItemState.Disabled;
            }
        }
    }
...
VSS.initClassPrototype(QueryEditor, {
    ...
    _linkFilter: null,
    _treeFilter: null,
    _linkFiltersHost: null,
    _treeFiltersHost: null,
    _linkFiltersLinkTypesAny: null,
    _linkFiltersLinkTypesSelected: null,
    _linkFilterModeSelect: null,
    _treeFilterModeSelect: null,
    _linkFiltersLinkTypes: null,
    _treeFiltersLinkTypes: null,
    _model: null,
    _menubar: null,
    _refreshDelegate: null
});

VSS.classExtend(QueryEditor, TfsContext.ControlExtensions);
